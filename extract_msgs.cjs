        ...
        fs.readdirSync(agentDir).forEach(f => {
            if (f.endsWith(".txt")) {
                ...
                try {
                    ...
                    if (content.includes("TD") || content.includes("목표") || content.includes("수정")) {
                        const lines = content.split('\n');
                        for (let i = 0; i < lines.length; i++) {
                            const line = lines[i];
                            if (line.includes("USER:") || line.includes("USER REQUEST") || line.includes("USER Objective") || line.includes("TD")) {
